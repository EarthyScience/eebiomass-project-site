...
export default function ThemeToggle({ className }: ThemeToggleProps) {
  ...
  const icons: { [key in ThemeType]: JSX.Element } = {
    ...
    system: (
      <svg width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        <rect width="12" height="8" x="2" y="2.67" rx="1.33" ry="1.33"/>
        <line x1="1.33" x2="14.67" y1="13.33" y2="13.33"/>
      </svg>
    ),
    dark: (
      <svg height="16" strokeLinejoin="round" viewBox="0 0 16 16" width="16">
  ...
  return (
    <div className={`theme-switcher ${className || ""}`}>
      <button className="theme-switch active">
        {icons[theme as ThemeType]}
      </button>
      <div className="dropdown-content">
        {themes.map(({ id }) => (
           <button key={id} onClick={() => setTheme(id)}>{icons[id]}</button>
        ))}
      </div>
    </div>
  );
